...
1. **Fast turns get no ack.** Given an eligible (sales-path) message, when the answer pipeline returns within the interim delay (`inbound_interim_delay_seconds`, default ~2s), then **no** interim ack is sent — only the real answer.
...
- [ ] **Add the delay setting** (AC: 5) — `Settings.inbound_interim_delay_seconds` (default ~2.0) + an `_effective_inbound_interim_delay()` resolver mirroring the other `_effective_inbound_*` helpers.
