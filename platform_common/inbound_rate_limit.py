"""Per-customer inbound rate limiting (shared across customer-facing gateways).
...
Originally story 12.103 (bot_gateway); promoted to ``platform_common`` so the
``user_gateway`` MTProto channel can apply the same policy (channel parity).
"""
